...
def extract_text_from_pdf_with_ocr(pdf_file):
    ...
    for page_number in range(len(pdf_document)):
        ...
        image_file = f"images/page_{page_number + 1}_processed.jpeg"
...
def extract_text():
    ...
    if file:
        ...
        extracted_text,files = extract_text_from_pdf_with_ocr(file_path)

        output_json = json.dumps(extracted_text, indent=4)

        return jsonify({'files':files, 'extracted_text': extracted_text})
